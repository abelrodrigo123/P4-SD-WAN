...
# --------- Conexión gRPC ---------
S1 = dict(device_id=1, grpc_ip='172.17.2.1', grpc_port=9559, json_path=JSON, p4rt_path=P4RT)
S2 = dict(device_id=2, grpc_ip='172.17.2.2', grpc_port=9559, json_path=JSON, p4rt_path=P4RT)

# --------- MACs y puertos físicos ---------
MAC_H1 = '02:fd:00:00:00:01'
MAC_H2 = '02:fd:00:00:01:01'

# En s1: eth1->h1, eth2->internet, eth4->MPLS hacia s2
# En s2: eth1->h2, eth2->internet, eth4->MPLS hacia s1
S1_ETH1, S1_ETH2, S1_ETH4 = '1', '2', '4'
S2_ETH1, S2_ETH2, S2_ETH4 = '1', '2', '4'
...
def program_switches():
    # --- S1 ---
    s1 = SimpleSwitchP4RuntimeAPI(**S1)
    s1.table_set_default('dmac', 'drop')
    # L2 por defecto: h2 por internet (eth2), h1 por eth1
    s1.table_add('dmac', 'forward', [MAC_H2], [S1_ETH2])
    s1.table_add('dmac', 'forward', [MAC_H1], [S1_ETH1])
    # Prioridad: si L4==6000, usar MPLS (eth4)
    for tbl, fld in [('prio_tcp_dst','dstPort'),
                     ('prio_tcp_src','srcPort'),
                     ('prio_udp_dst','dstPort'),
                     ('prio_udp_src','srcPort')]:
        s1.table_add(tbl, 'prio_forward', [PRIO_L4], [S1_ETH4])  # h1->h2
        s1.table_add(tbl, 'prio_forward', [PRIO_L4], [S1_ETH1])  # regreso h2->h1

    # --- S2 ---
    s2 = SimpleSwitchP4RuntimeAPI(**S2)
    s2.table_set_default('dmac', 'drop')
    # L2 por defecto: h1 por internet (eth2), h2 por eth1
    s2.table_add('dmac', 'forward', [MAC_H1], [S2_ETH2])
    s2.table_add('dmac', 'forward', [MAC_H2], [S2_ETH1])
    # Prioridad simétrica por MPLS (eth4)
    for tbl, fld in [('prio_tcp_dst','dstPort'),
                     ('prio_tcp_src','srcPort'),
                     ('prio_udp_dst','dstPort'),
                     ('prio_udp_src','srcPort')]:
        s2.table_add(tbl, 'prio_forward', [PRIO_L4], [S2_ETH4])  # h2->h1
        s2.table_add(tbl, 'prio_forward', [PRIO_L4], [S2_ETH1]) # regreso h1->h2

    print('[done] Reglas instaladas (L2 + prioridad TCP/UDP puerto 6000 por eth4).')
